...
def create_message(msg: MessageCreate, db: Session = Depends(get_session)):
    logger.info(f'Scheduling the "{msg.content}" message for the "{msg.scheduled_time}" time')
...
def get_message(msg_id: int, db: Session = Depends(get_session)):
    logger.info(f'Fetching message with id={msg_id}')
    msg = get_message_by_id(db, msg_id)
    if not msg:
        logger.warning(f'Message with id={msg_id} not found')
...
def send_message(msg_id: int, db: Session = Depends(get_session)):
    logger.info(f'Attempting to send message with id={msg_id}')
    msg = get_message_by_id(db, msg_id)
    if not msg:
        logger.warning(f'Message with id={msg_id} not found')
        raise HTTPException(status_code=404, detail='Message not found')
    if msg.sent:
        logger.warning(f'Message with id={msg_id} has already been sent')
        raise HTTPException(status_code=400, detail='Message already sent')

    try:
        send_scheduled_message(db, msg)
        logger.info(f'Message with id={msg_id} sent successfully')
    except Exception as e:
        logger.exception(f'Failed to send message with id={msg_id}: {e}')
